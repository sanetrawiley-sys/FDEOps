...
For substantial work, keep the implementation plan and progress in the existing project task record. A bound engagement may hold a short confirmed summary under `## Implementation checkpoint` in `context.md`. Put the next action and source task path/ID first, followed by repository/revision and dirty state, completed and pending checks, blocker and update time. Replace the previous checkpoint or close it when work finishes; do not copy a second backlog.
